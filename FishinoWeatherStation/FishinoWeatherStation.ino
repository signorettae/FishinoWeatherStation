...
const char * APIKEY  = "" ; // Put here your Pushetta API key. Inserire qui la propria chiave API di Pushetta.
const char * CHANNEL  = ""; // Put here your Pushetta channel name. Inserire qui il nome del proprio canale Pushetta.
//
//
...
void connectToAp(void) {
 ...
 // Setta l'indirizzo IP o avvia il server DHCP
#ifdef IPADDR
 ...
 // Aspetta il completamento della connessione
...
uint32_t wifiGetUtc() {
	...
	for (i = 0; i < 10; i++) {
            ...
            //Add 3600 secs because we are in GMT zone +1
            //Aggiunge 3600 secondi pechè siamo nella zona GMT +1
			retval = epoch2k + 3600;
...

{
 ...
 // Se le ore sono meno di 10 stampa prima uno 0 e poi il valore
	if (dt.hour() < 10) {
			Serial.print("0");
  ...
  // Se i minuti sono meno di 10 stampa prima uno 0 e poi il valore
	if (dt.minute() < 10) {
			Serial.print("0");
    ...
    // Se i secondi sono meno di 10 stampa prima uno 0 e poi il valore
...
void sendFile(FishinoWebServer& web, const char* filename) {
	...
	if (!filename) {
     ...
     //If it opens the file, send it through the web server
          //Se riesce ad aprire il file, invialo attraverso il webserver
...
////ITALIAN  STATUS HANDLER: create a .json with real time data. HANDLER DI STATO IN ITALIANO: crea un file .json con i dati in tempo reale
bool statusHandler(FishinoWebServer& web) {
	...
	FishinoClient& client = web.getClient(); //Create a client object. Crea un oggetto client
	client.println("{");
...
//Begin the I/O pins and the sensors
//Inizializza i pin di I/O e i sensori
	...
	// Using interrupts means you do not have to check for pin being set continously, chip does that for you and notifies your code
	// Usere gli interrupt significa che non è necessario controllare lo stato del pin in continuazione, il chip lo fa in automatico 
...
void loop(void) {
	...
	if (AS3935IrqTriggered) {
			...
			// here we go into loop checking if interrupt has been triggered, which kind of defeats
			// returned value is bitmap field, bit 0 - noise level too high, bit 2 - disturber detected, and finally bit 3 - lightning!
      //Adesso controlliamo nel loop se è stato rilevato un interrupt, di che tipo si tratta
      // i valori son mappati, bit 0 - soglia del rumore troppo alta, bit 2 - rilevato disturbo, e bit 3 - fulmine!  
...
#endif
	EVERY(300000) {
  //EVERY 300000 ms = 300 s = 5 min: save data on datalog file with real time data 
  //Ogni 300000 ms = 300 s = 5 min: salva i dati istantanei sul datalog
		saveData(message.c_str());

	}

	EVERY(3600000) { 
  //EVERY 3600000 ms = 3600 s = 1 hour: send push notification with real time data 
  //Ogni 3600000 ms = 3600 s = 1 ora: invia una notifica push con i dati in tempo reale
